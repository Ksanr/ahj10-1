...
function addTextPostToDOM(text, latitude, longitude) {
    ...
    dateSpan.textContent = post.timestamp.toLocaleString('ru-RU');

    const coordsSpan = document.createElement('span');
    coordsSpan.classList.add('post-coordinates');
    coordsSpan.textContent = `📍 ${latitude.toFixed(5)}, ${longitude.toFixed(5)}`;

    header.append(dateSpan, coordsSpan);

    const textDiv = document.createElement('div');
    textDiv.classList.add('post-text');
    textDiv.textContent = text;

    postElement.append(header, textDiv);
...
function addAudioPostToDOM(audioBlob, latitude, longitude) {
    ...
    postElement.append(header, audioElem);
    timelineContainer.prepend(postElement);
}
...
function addVideoPostToDOM(videoBlob, latitude, longitude) {
    ...
    postElement.append(header, videoElem);
    timelineContainer.prepend(postElement);
}
